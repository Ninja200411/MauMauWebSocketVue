<template>
  <div>
    ...
    <div class="controls-over">
      <h3>Controls:</h3>
      <button v-if="!isConnected" @click="connect" :disabled="isConnected">
      ...
      <form
        v-if="isConnected && user === null"
        @submit.prevent="send(MessageType.JOIN, message.sessionId!, payload)"
      >
      ...
      <button
        v-if="isConnected && user !== null"
        @click="send(MessageType.LEAVE, user?.sessionId!)"
      >
        Leave the session
      </button>
      <button
        v-if="isConnected && !isStarted && user !== null"
        @click="send(MessageType.START, user.sessionId!)"
      >
        Start the game
      </button>
    </div>
    <div class="game">
      ...
      <div class="game">
        <div class="stapel">
          ...
          <img
            v-if="isStarted && isAtRow"
            @click="send(MessageType.DRAWCARD, user?.sessionId!)"
            src="../pictures/Rückseite.png"
          />
          <img
            v-if="topCard !== null"
            :src="
        ...
        <div v-if="isConnected && user !== null && isStarted" class="card-box">
          <div v-for="c in cards" class="card-img">
            <img
              :class="isAtRow ? 'auswahl-img' : ''"
              @click="
                isAtRow
                  ? c?.number !== CardNumber.BUBE
                    ? send(MessageType.PLAYCARD, user?.sessionId!, {
                        number: c?.number,
                        symbol: c?.symbol,
                      })
                    : null
            ...
            <div v-if="c.number === CardNumber.BUBE" class="card-img-auswahl">
              <button>
                <img
                  src="../pictures/Pik.png"
                  @click="
                    isAtRow
                      ? send(MessageType.PLAYCARD, user?.sessionId!, {
                          number: c?.number,
                          symbol: c?.symbol,
                          color: CardSymbol.PIK,
                        })
                      : null
                  "
                />
              ...
              <button>
                <img
                  src="../pictures/Herz.png"
                  @click="
                    isAtRow
                      ? send(MessageType.PLAYCARD, user?.sessionId!, {
                          number: c?.number,
                          symbol: c?.symbol,
                          color: CardSymbol.HERZ,
                        })
                      : null
                  "
                />
              ...
              <button>
                <img
                  src="../pictures/Karo.png"
                  @click="
                    isAtRow
                      ? send(MessageType.PLAYCARD, user?.sessionId!, {
                          number: c?.number,
                          symbol: c?.symbol,
                          color: CardSymbol.KARO,
                        })
                      : null
                  "
                />
              ...
              <button>
                <img
                  src="../pictures/Kreuz.png"
                  @click="
                    isAtRow
                      ? send(MessageType.PLAYCARD, user?.sessionId!, {
                          number: c?.number,
                          symbol: c?.symbol,
                          color: CardSymbol.KREUZ,
                        })
                      : null
                  "
                />
  ...
  function onMessage(event: MessageEvent) {
    ...
    switch (data.type) {
      ...
      case MessageType.LEFT:
        ...
        const newUsers = users.value!.filter(
      ...
      case MessageType.YOUPLAYEDCARD:
        ...
        cards.value = cards.value.filter(
          (card) =>
            !(
              card.number === data.payload.number &&
              card.symbol === data.payload.symbol
            ),
        );
        console.log(cards.value);
        break;
